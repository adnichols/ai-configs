...
- Mark each phase complete only after its verification passes.
...
3. If verification passes, immediately flip the phase checkbox from `- [ ]` to `- [x]` in `## Progress`.
4. If verification reports an in-scope failure, fix it and rerun the phase `### Verify` steps before advancing.
5. If implementation required a decision or revealed a constraint, append a structured entry to `## Decisions / Deviations Log` in the plan file.
