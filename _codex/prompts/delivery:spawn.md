---
description: From any session, create a Herdr worktree from a freeform request and start the delivery workflow there
...
5. **Agent / runtime** — default starts the recorded delivery runtime agent in the new worktree (Pi or Codex per the worker/role). Use `--no-agent` only if the operator asked for worktree-only setup.
...
- Child agent prompted to follow the brief (unless `--no-agent`)
