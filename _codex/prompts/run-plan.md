...
This wrapper is only the ergonomic `/run-plan` entry point. Do not run a shortened workflow here. Follow the `run-plan` skill so scoped implementation, verification, active-harness reviewer-subagent pre-PR review, base freshness, PR creation, and post-PR monitoring all stay in the single source of truth.
