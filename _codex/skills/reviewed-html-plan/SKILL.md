...
The planning agent stops before product-code execution and may edit only the plan artifact(s). Planning-only is the default: do not create a delivery ledger, do not run `delivery init`/`bootstrap`/`arm`, and do not launch an implementation pane. An existing external delivery ledger retains its runtime owner. Do not advance its stages from this native Codex planning workflow. Without a ledger, stop after the execution-ready plan.
...
- Request one independent read-only native planner using the Codex runtime contract. Give the raw plan and repository evidence, and require an explicit readiness verdict.
- Use the parallel `oracle-consultation` skill for one consequential unresolved technical choice. It does not replace browser feedback or the independent readiness review.
