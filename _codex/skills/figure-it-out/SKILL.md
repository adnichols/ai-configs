---
name: figure-it-out
description: "Develop an evidence-based approach to complex work when no existing playbook fits."
---
...
Open a todolist whose first item is to read the Principles section of the **adn-mode** skill. Then add the phases below as todos.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated, opinionated candidates and an independent read-only judge under the Codex runtime contract. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
- Decide what fans out. Parallelize only across genuine seams, and give each read-only investigator a bounded evidence scope (the **separate-before-serializing-shared-state** principle skill). Don't over-fan.
