...
For a planning-only task, a published execution-ready plan with no pending feedback completes the native planning lifecycle. Release this task's listener/monitor ownership using its documented stop procedure after draining acknowledged work. Preserve durable server state and other owners. Return the review URL and readiness evidence. Do not wait indefinitely for an implementation task that the user did not request.
