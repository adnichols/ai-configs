...
1. **P2 — IN_PLAN — Gate-blocking: yes.** C-Core reconciliation only examines the first 500 documents, so it can create a duplicate report instead of detecting/reusing the immutable same-title report.
   ...
   - Evidence: [pi_analytics_collector.py](../../../_hermes/default/scripts/pi_analytics_collector.py#L321) hard-caps the list to 500; [reconcile_existing](../../../_hermes/default/scripts/pi_analytics_collector.py#L348) treats that partial result as exhaustive. The supported CLI exposes pagination via `--cursor`; [tests](../../../scripts/tests/test_pi_analytics_ccore_publish.py#L56) only model a single-page list.
...
2. **P2 — IN_PLAN — Gate-blocking: yes.** A malformed or unreadable collector configuration exits as `publish_failed` without updating `last-run.json`, leaving `--status --json` to report an earlier success.
   ...
   - Evidence: [collector](../../../_hermes/default/scripts/pi_analytics_collector.py#L421) loads config before any status write; the only top-level handling at [main](../../../_hermes/default/scripts/pi_analytics_collector.py#L496) prints an error and returns. Existing tests cover failures only after a mocked successful config load at [test_pi_analytics_deploy.py](../../../scripts/tests/test_pi_analytics_deploy.py#L143).
