...
1. **P2 — IN_PLAN — Gate-blocking: yes.** Partial Doct-close failure duplicates the visible reply on retry.
   ...
   - Reachable path: [`process_claim`](/home/anichols/code/ai-configs/_hermes/default/scripts/pi_analytics_action.py:452) records the ledger first; [`finish_claim`](/home/anichols/code/ai-configs/_hermes/default/scripts/pi_analytics_action.py:386) posts a reply before ack/resolve; failure releases the claim, and redelivery enters `finish_claim` again.
   ...
   - Evidence: the retry test only models failure of the initial reply; it does not model reply-success/ack-failure or reply-success/resolve-failure ([test](/home/anichols/code/ai-configs/scripts/tests/test_gm_pi_analytics_action.py:215)).
...
2. **P2 — IN_PLAN — Gate-blocking: yes.** The restricted path persists the full, untrusted claim payload in a long-lived, normally world-readable state file.
   ...
   - Reachable path: [`start_analytics_worker`](/home/anichols/code/ai-configs/_hermes/default/scripts/gm_plan_comment_listener.py:206) serializes the complete payload through [`_write_claim_file`](/home/anichols/code/ai-configs/_hermes/default/scripts/gm_plan_comment_listener.py:197); `write_text` creates the final JSON using the process umask, and neither the claim nor its parent directory is restricted or cleaned up.
   ...
   - Evidence: the privacy test proves only ledger and outgoing Doct argv content, not the listener-created claim JSON ([test](/home/anichols/code/ai-configs/scripts/tests/test_gm_pi_analytics_action.py:251)). The fixture’s injected `privatePrompt` would be written verbatim by the listener.
