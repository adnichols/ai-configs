...
- Stop `/cursor-refresh-models` after Pi emits `session_shutdown`, so an asynchronous catalog refresh cannot use a stale extension API or command context after `/reload`, `/new`, `/resume`, `/fork`, or another session replacement.
- Default the interactive `cursor_ask_question` bridge off. Set `PI_CURSOR_ASK_QUESTION=1` to enable it explicitly.
