...
export default async function (pi: CursorExtensionApi) {
	// Pi invalidates this extension API and every context after session replacement.
	// Async refresh work must stop rather than touch the old runtime when that happens.
	...
	pi.registerCommand("cursor-refresh-models", {
		...
		handler: async (_args, ctx) => {
			...
			const configuredApiKey = await ctx.modelRegistry.getApiKeyForProvider("cursor");
			if (!runtimeIsCurrent) return;
			const apiKey = resolveCursorApiKey(configuredApiKey);
