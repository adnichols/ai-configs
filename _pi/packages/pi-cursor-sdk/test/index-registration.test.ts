...
describe("extension registration and discovery", () => {
	beforeEach(async () => {
		...
		// Most registration tests exercise the optional bridge explicitly.
	...
	it("defaults the Cursor question tool off and enables it only with PI_CURSOR_ASK_QUESTION=1", async () => {
	...
	it("abandons a model refresh when its extension runtime shuts down", async () => {
		const startupModels = [makeProviderModelConfig("composer-2", { name: "Cursor Composer 2" })];
		const refreshedModels = [makeProviderModelConfig("gpt-5.5@1m", { name: "GPT-5.5 @ 1m" })];
		let resolveRefresh!: (models: typeof refreshedModels) => void;
		const refreshModels = new Promise<typeof refreshedModels>((resolve) => {
			resolveRefresh = resolve;
		});
		mockedDiscover.mockResolvedValueOnce(startupModels).mockImplementationOnce(() => refreshModels);
		const pi = createExtensionPi();
		await extensionFactory(pi);
		const notify = vi.fn();

		const pendingRefresh = pi.runCommand(
			"cursor-refresh-models",
			"",
			createExtensionCommandContext({
				hasUI: true,
				model: undefined,
				modelRegistry: { getApiKeyForProvider: vi.fn().mockResolvedValue("registry-key") } as never,
				ui: { notify },
			}),
		);
		await vi.waitFor(() => expect(mockedDiscover).toHaveBeenCalledTimes(2));
		await pi.runSessionShutdown({ reason: "reload" });
