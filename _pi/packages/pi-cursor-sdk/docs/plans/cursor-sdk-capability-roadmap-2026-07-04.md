...
| Optional Cursor question prompt | **Implemented** | `PI_CURSOR_ASK_QUESTION=1` enables the otherwise-disabled `cursor_ask_question` while preserving the rest of the bridge; `src/cursor-question-tool.ts` and registration/bridge tests own the contract. |
