...
export const safePiVccLogJson = (value: unknown): unknown => {
	...
	if (value instanceof Map) return Object.fromEntries([...value].map(([key, raw]) => [String(key), safePiVccLogJson(raw)]));
	if (!value || typeof value !== "object") return value;
	const output: Record<string, unknown> = {};
	for (const [key, raw] of Object.entries(value as Record<string, unknown>)) {
		output[key] = SECRET_KEY_PATTERN.test(key) ? "[redacted]" : safePiVccLogJson(raw);
	}
	return output;
};
...
export const logPiVccEvent = (event: string, data: Record<string, unknown> = {}) => {
	try {
		...
		// Logging must never interfere with compaction.
	}
};
