...
ADN_RUNTIME_MARKER:adn-mode:46756f89270d7e7dcb8c28c90fd0f957ade4ce2c

# Poteto mode (Pi)

This is the Pi-adapted adn-mode. It shadows the shared `adn-mode` for Pi sessions only (`~/.pi/agent/skills` loads before `~/.agents/skills`). Doctrine is identical; the harness contract and model routes below are Pi's. Playbooks and references are the shared ADN payload.
...
Remaining triggers:
- Pi has no `eval` tool. Do not look for one. Use `read`, `grep`, and `bash` for inspection; `edit` for surgery; `write` for new files; `bash` for a committed lever. The **Eval** playbook below is a blind skill A/B test, not a tool.
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply** every single time, regardless of todo state. Agent-facing prose also follows the **skill-creator** skill for authoring SKILL.md files.
...
- Design exploration or architecture (the Grok role): `model: "xai/grok-4.6:high"`.
- Second-family design exploration (the Kimi architect role): `model: "synthetic/hf:moonshotai/Kimi-K3:max"`.
- Adversarial or second-family review (the Kimi reviewer role): `model: "synthetic/hf:moonshotai/Kimi-K3:high"`.
- Implementation slices (edits, tests, multi-file code): `model: "xai/grok-4.6:high"`.
- Mechanical bulk work that needs a cheap route: `model: "deepinfra/deepseek-ai/DeepSeek-V4-Flash-0731"`.

The default code model is `xai/grok-4.6:high` and the default prose and judgment model is `synthetic/hf:moonshotai/Kimi-K3`. Never omit `model` on an implementation spawn expecting the right route: omitting it inherits the parent session model, including a temporary picker choice. Parent `/model` does not retarget a running child.

Run agents in the background by default; Pi notifies on completion. Use foreground only when the very next action depends on the result. If a child tool-loops or reports production bugs instead of finishing, stop it (`subagents_stop` or the Agent stop path) and respawn with the explicit model. Do not keep messaging it.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run with git merge-when-ready. `playbooks/shipping.md`.
...
- **Autopilot-stack.** A queue of changes built and verified with full autonomy, delivered as one linear reviewed git stack the operator lands herself ("autopilot-stack", "stack them, don't ship", "build the stack, I'll land it"). `playbooks/autopilot-stack.md`.
- **Session pickup.** Resuming or taking over a prior agent's in-flight work from a transcript, session URL, or pushed branch. `playbooks/session-pickup.md`.
