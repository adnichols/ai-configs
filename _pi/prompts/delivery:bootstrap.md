...
- Completeness is on-request. If the operator asked for a plan walk, at `COMPLETENESS_REVIEW` run `delivery completion-review`, read the visible labeled Pi/Grok 4.6 tab, fix in-plan findings, `--rerun` until `VERDICT: COMPLETE`, then `--accept`. Skip this when they did not ask; it does not block merge readiness.
