...
- Apply the canonical `planning-workflow` integration-integrity planning contract: when discovery finds an exact untyped contract or distributed production behavior, include the triggered Contract and distributed-integration inventory and source-search evidence; record `None identified, based on <source search>` when neither trigger applies.
