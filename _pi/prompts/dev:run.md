...
Use the repository-owned read-only `oracle` subagent proactively when the plan and targeted inspection leave one consequential implementation decision genuinely ambiguous: competing architecture or ownership options, a hard-to-reverse exact contract/schema/migration choice, conflict with locked plan decisions, or a newly exposed tradeoff that may require a plan pivot. Give Oracle a bounded decision packet with constraints, evidence, credible options, the driving agent's recommendation, uncertainty, and one narrow question. Verify its claims and record whether the recommendation was accepted or rejected. Oracle cannot edit, authorize scope expansion, make a product choice, replace the required phase `reviewer`, or serve as routine discovery.
