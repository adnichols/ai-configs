...
Use the repository-owned read-only `oracle` subagent proactively when the plan and targeted inspection leave one consequential implementation decision genuinely ambiguous: competing architecture or ownership options, a hard-to-reverse exact contract/schema/migration choice, conflict with locked plan decisions, or a newly exposed tradeoff that may require a plan pivot. Do not wait for the operator to request Oracle. Launch with only `subagent_type: "oracle"`, a short description, and the decision prompt; omit caller-side `model`, `thinking`, `inherit_context`, and `isolation` (`inherit_context: false` and `isolation: "worktree"` are workflow violations). Give Oracle a bounded decision packet with constraints, evidence, credible options, the driving agent's recommendation, uncertainty, and one narrow question ending with `?`. Verify its claims and record disposition (`accepted` / `partially-accepted` / `rejected` / `escalated`) with why. Oracle cannot edit, authorize scope expansion, make a product choice, replace the required phase `reviewer`, or serve as routine discovery.
