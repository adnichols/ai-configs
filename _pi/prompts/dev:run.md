...
Delegation to `developer-mid` must include target files, desired behavior, relevant evidence, and verification commands. Keep all implementation delegation at medium effort. If repeated bounded attempts fail to converge, stop and return control to the user rather than escalating reasoning effort.
...
- When authorized, audit the fixed candidate branch/diff for sibling instances in the named family: the same assumption, edge case, API contract, missing validation, lifecycle/state transition, analogous callsites, and tests that should have failed but did not. Use the existing read-only `reviewer` as the primary adversarial leg. Add Claude Code only when the repo/harness high-risk second-reviewer trigger or an explicit override applies, using the installed `claude-code-review` plus `herdr-reviewers`; otherwise record the primary-only classification rather than inventing a second leg. Run one bounded adversarial pass over the named family, allow the sole implementation authority one bounded fix attempt for in-scope findings, and run the same applicable reviewer pass once after fixes. This route has no PR prerequisite. Do not restart the ordinary three-round budget or review until clean.
