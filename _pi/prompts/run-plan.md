...
Before delegating code, use `Explore`/`explore` for broad discovery and pass `developer-mid` a bounded packet with target files, intended behavior, relevant evidence, and verification commands. Keep all implementation delegation at medium effort. If repeated bounded attempts fail to converge, stop and return control to the user rather than escalating reasoning effort.
