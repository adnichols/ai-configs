...
Keep GPT-5.6 Sol medium as the normal `/run-plan` OpenAI parent and code-writing route. For planning/orchestration-heavy, review-synthesis-heavy, UI-design-heavy, or long test/debug-loop runs, use one explicit scoped GLM path instead of rerouting unrelated sessions:

1. The operator may switch the active Pi scoped model to `opencode/glm-5.2` before invoking this wrapper.
2. A GPT-5.6 Sol parent may delegate a bounded planning/orchestration packet to `orchestrator-glm`.
