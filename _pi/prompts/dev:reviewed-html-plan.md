...
- During plan authoring or revision, proactively consult the read-only `oracle` subagent once as advisory decision support when targeted evidence leaves a consequential technical approach, ownership boundary, compatibility tradeoff, or conflict with locked decisions genuinely ambiguous. Do not wait for the operator to request Oracle. Launch with only `subagent_type: "oracle"`, a short description, and the decision packet; omit caller-side `model`, `thinking`, `inherit_context`, and `isolation` (`inherit_context: false` and `isolation: "worktree"` are workflow violations). Give it a bounded decision packet (constraints, evidence, options, current recommendation, uncertainty, one `?` question), verify its claims, and record disposition (`accepted` / `partially-accepted` / `rejected` / `escalated`) with why when it affects the plan. Oracle cannot make product choices, authorize scope expansion, or replace any readiness gate.
