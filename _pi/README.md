...
The exact `opencode/grok-4.5` model uses an absolute policy rather than the shared percentage backstop. The extension requests pi-vcc compaction at **180,000 tokens** and advertises a **200,000-token** context window. Other providers and Grok model IDs, including Pi's built-in `xai/grok-4.6`, retain the normal 60%/75%/80% percentage policy.
