...
`model-allowlist.ts` and the Pi model cycle expose exactly six routes with pinned reasoning: `openai-codex/gpt-5.6-terra:high`, `openai-codex/gpt-5.6-luna:xhigh`, `openai-codex/gpt-5.6-sol:medium`, `xai/grok-4.5:high`, `opencode/deepseek-v4-flash:high`, and `synthetic/hf:moonshotai/Kimi-K3:high`. The allowlist filters the live model catalog by bare provider/model identity, while the managed `enabledModels` scope pins these reasoning levels; verify both after a Pi upgrade.
