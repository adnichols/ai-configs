...
`model-allowlist.ts` exposes twelve routes. Seven are in the managed Pi model cycle with pinned reasoning; the default execution route is `deepinfra/deepseek-ai/DeepSeek-V4-Flash-0731:high`, followed by `openai-codex/gpt-5.6-terra:high`, `openai-codex/gpt-5.6-luna:xhigh`, `openai-codex/gpt-5.6-sol:medium`, `xai/grok-4.5:high`, `cursor/grok-4.5:high`, and `opencode/deepseek-v4-flash:high`. The bare `cursor/grok-4.5` allowlist identity intentionally excludes Cursor's `:fast` and `:slow` transport variants, and install persists `fastDefaults.grok-4.5=false` so the unsuffixed cycle pin stays plain instead of Cursor's catalog-default Fast variant. Five additional routes are available for direct selection but intentionally remain outside `enabledModels`: `synthetic/hf:moonshotai/Kimi-K3`, `fireworks/accounts/fireworks/models/deepseek-v4-flash-0731`, `fireworks/accounts/fireworks/models/kimi-k3`, `deepinfra/zai-org/GLM-5.2`, and `deepinfra/moonshotai/Kimi-K3`. DeepInfra Kimi K3 is also upserted via `_pi/models.json` so it is present at startup; `pi-deepinfra`'s fallback catalog does not include it. The allowlist filters the live model catalog by bare provider/model identity, while the managed `enabledModels` scope defines the seven-model cycle and pins its reasoning levels; verify both after a Pi upgrade. `@aliou/pi-synthetic` remains installed; Synthetic Kimi K3 is allowlisted for the extended model list but is not in the managed cycle.
