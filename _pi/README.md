...
`install.sh --pi` now enforces `deepinfra/deepseek-ai/DeepSeek-V4-Flash-0731` as the Pi default execution model (with `openai-codex/gpt-5.6-terra` kept in the model cycle for planning/review), while the web-search summary route stays on GPT-5.6 Terra. The driving Pi session is the primary code-writing route. The planning and review agents use GPT-5.6 Terra medium (pegged by agent frontmatter), and the read-only scout uses GPT-5.6 Terra low. `pi-prewalk` can hand mechanical implementation off to the deepinfra DeepSeek Flash execution model at the first `edit`/`write` once a strong model has committed to a plan. GPT-5.6 Sol remains available in Pi-scoped model routes for execution workflows, while GLM-5.2 is removed; GPT-5.4 and GPT-5.4-mini are also retired exactly from Pi-owned agents, managed `openai-codex` model entries, and Pi settings aliases while caller-owned providers/models remain untouched.
...
`model-allowlist.ts` exposes eleven routes. Eight are in the managed Pi model cycle with pinned reasoning; the default execution route is `deepinfra/deepseek-ai/DeepSeek-V4-Flash-0731:high`, followed by `openai-codex/gpt-5.6-terra:high`, `openai-codex/gpt-5.6-luna:xhigh`, `openai-codex/gpt-5.6-sol:medium`, `xai/grok-4.5:high`, `cursor/grok-4.5:high`, `opencode/deepseek-v4-flash:high`, and `synthetic/hf:moonshotai/Kimi-K3:high`. The bare `cursor/grok-4.5` allowlist identity intentionally excludes Cursor's `:fast` and `:slow` transport variants. Three additional routes are available for direct selection but intentionally remain outside `enabledModels`: `fireworks/accounts/fireworks/models/deepseek-v4-flash-0731`, `fireworks/accounts/fireworks/models/kimi-k3`, and `deepinfra/zai-org/GLM-5.2`. The allowlist filters the live model catalog by bare provider/model identity, while the managed `enabledModels` scope defines the eight-model cycle and pins its reasoning levels; verify both after a Pi upgrade.
...
Required Pi plan and code reviews use the repository-owned `reviewer` Pi subagent (`openai-codex/gpt-5.6-terra`, medium reasoning). The coordinating Pi session gives it a bounded, read-only review packet and captures its result in the normal review artifact. A Herdr delivery run also opens a visible adjacent Pi session on `xai/grok-4.5:high` for the separate plan-completeness loop: the driving agent addresses its in-plan findings and requests rereview until it returns `COMPLETE`. That visible reviewer is read-only and does not replace the active-harness code-review gate. Separate Codex or Claude Code sessions remain unnecessary.
...
- `pi-prewalk` — lets a strong model commit to a plan, then hands mechanical implementation off to a fast/cheap execution model at the first `edit`/`write` (todo-gated one-way switch). Arm with `--prewalk`, `--prewalk-into <model>`, or `/prewalk`.
