...
`model-allowlist.ts` exposes nine routes. Seven are in the managed Pi model cycle with pinned reasoning: `openai-codex/gpt-5.6-terra:high`, `openai-codex/gpt-5.6-luna:xhigh`, `openai-codex/gpt-5.6-sol:medium`, `xai/grok-4.5:high`, `cursor/grok-4.5:high`, `opencode/deepseek-v4-flash:high`, and `synthetic/hf:moonshotai/Kimi-K3:high`. The bare `cursor/grok-4.5` allowlist identity intentionally excludes Cursor's `:fast` and `:slow` transport variants. Two additional built-in Fireworks routes are available for direct selection but intentionally remain outside `enabledModels`: `fireworks/accounts/fireworks/models/deepseek-v4-flash-0731` and `fireworks/accounts/fireworks/models/kimi-k3`. The allowlist filters the live model catalog by bare provider/model identity, while the managed `enabledModels` scope defines the seven-model cycle and pins its reasoning levels; verify both after a Pi upgrade.
