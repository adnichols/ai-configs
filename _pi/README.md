...
`model-allowlist.ts` exposes eleven routes. Eight are in the managed Pi model cycle with pinned reasoning; the default execution route is `deepinfra/deepseek-ai/DeepSeek-V4-Flash-0731:high`, followed by `openai-codex/gpt-5.6-terra:high`, `openai-codex/gpt-5.6-luna:xhigh`, `openai-codex/gpt-5.6-sol:medium`, `xai/grok-4.6:high`, `cursor/grok-4.6:high`, `opencode/deepseek-v4-flash:high`, and `synthetic/hf:moonshotai/Kimi-K3:high`. The bare `cursor/grok-4.6` allowlist identity intentionally excludes Cursor's `:fast` and `:slow` transport variants, and install persists `fastDefaults.grok-4.6=false` so the unsuffixed cycle pin stays plain instead of Cursor's catalog-default Fast variant. Three additional routes are available for direct selection but intentionally remain outside `enabledModels`: `fireworks/accounts/fireworks/models/deepseek-v4-flash-0731`, `fireworks/accounts/fireworks/models/kimi-k3`, and `deepinfra/zai-org/GLM-5.2`. The allowlist filters the live model catalog by bare provider/model identity, while the managed `enabledModels` scope defines the eight-model cycle and pins its reasoning levels; verify both after a Pi upgrade. `@aliou/pi-synthetic` remains installed so the Synthetic Kimi K3 cycle pin is available after install.
