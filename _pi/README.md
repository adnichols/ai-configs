...
`model-allowlist.ts` and the Pi model cycle expose exactly four routes with pinned reasoning: `openai-codex/gpt-5.6-terra:high`, `openai-codex/gpt-5.6-luna:max`, `openai-codex/gpt-5.6-sol:medium`, and `xai/grok-4.5:high`. The allowlist filters the live model catalog by bare provider/model identity, while the managed `enabledModels` scope pins these reasoning levels; verify both after a Pi upgrade.
