...
- a started review remains an active, visibly running tool/subprocess row with its job ID and supervisor PID, keeps a footer status visible, and returns only after the reviewer exits so Pi cannot look finished while the reviewer is still working,
- accepted jobs run under a detached supervisor and survive an interrupted tool call, Pi turn cancellation, session replacement, workspace/session shutdown, extension reload, and Pi exit,
...
The repo-owned `codex-review` extension similarly provides `codex_review` for required Pi Codex reviews. A started review remains an active, visibly running tool/subprocess row with its job ID and PID, keeps a footer status visible, and returns only after Codex exits; if the visible tool call is interrupted while the managed job remains eligible, completion is reported automatically. It launches the canonical `codex-review-partner` wrapper, captures only Codex's final message, validates the workflow-specific verdict profile, keeps JSONL/stderr/state evidence under `~/.pi/agent/cache/codex-review/`, and sends exactly one completion follow-up for notification-eligible detached terminal outcomes. Jobs are bound to the originating Pi session: a reload or restart of that same session may recover its completion, but a new, resumed, or forked different session never receives the old session's completion or an automatic LLM turn. Graceful shutdown reaps active private-session process groups without a new turn, while next-start reconciliation classifies abrupt interruption for explicit `list`/`status` recovery. Linux and macOS are supported without an external `setsid` command: the installed Python identity adapter uses kernel-backed `/proc` or Darwin `libproc` evidence, and cleanup fails closed on unsupported capability or PID/start/boot/SID mismatch. Non-Pi consumers continue to use `run-review.sh` with an explicit `generic-implementation` or `generic-plan` verdict profile; pair mode remains profile-less.

For bounded review-stack installation, use `scripts/install-pi-transactionally.sh`. It snapshots complete `~/.pi` plus the six maintained shared-skill directories (`autoreview` plus its `pre-pr-implementation-review` compatibility alias, `claude-code-review`, and the three other review-stack skills), invokes `install.sh --pi-review-stack`, runs the non-mutating `scripts/verify-pi-install.sh --scope pi-review-stack --check-only` verifier, checks recursive source parity, runs the installed identity-helper and supervisor capability/permission preflight, then checks the installed fake-launcher host path. Any failure restores the exact snapshot. This mode intentionally skips Pi packages, pi-vcc checks, broad shared-skill synchronization, retired-runtime cleanup, and project-link enforcement.

If a macOS review reports a process-inspection or cleanup failure, inspect the job's stderr/status/identity paths and run `python3 ~/.agents/skills/codex-review-partner/scripts/process_identity.py preflight` plus `python3 ~/.agents/skills/codex-review-partner/scripts/review_supervisor.py --preflight`. Do not install a PATH-only `setsid` workaround or bypass `codex_review`; unresolved cleanup uncertainty must be cleared before retrying.
