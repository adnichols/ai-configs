...
test("75 mixed concurrent jobs complete once without registry leaks", async (t) => {
  ...
  const modes = ["success", "success", "success", "malformed-success", "classified-failure", "no-artifact", "nonzero-no-artifact"];
  const starts = [];
  ...
  const expectedSuccesses = Array.from({ length: 75 }, (_, i) => modes[i % modes.length]).filter((mode) => mode === "success").length;
...
test("concurrent cancellation and shutdown leave no live child pids", async (t) => {
  ...
  const jobs = await Promise.all(starts);
  await Promise.all(jobs.slice(0, 6).map((job) => manager.cancel(job.jobId)));
