...
A provider name is not a profile selection. In particular, `paseo run --provider omp` does not select an agent profile named `omp`. When its model is omitted, Paseo resolves one from the provider catalog and may pass that model explicitly to OMP instead of using OMP's configured `default` role.
...
The CLI currently has no profile flag. When only the CLI is available, read the target host's current `agentProfiles` configuration (local hosts store it in `~/.paseo/config.json`), select the named profile by exact name, and materialize its fields into `paseo run`. Do not choose or replace the model independently. For a remote host, read that host's configuration rather than the caller's local file.
