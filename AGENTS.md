...
Pi does not install a global `APPEND_SYSTEM.md`. Conditional implementation procedures load through skills: `integration-integrity`, `safe-git-index`, and `oracle-consultation`. `cmd-create-pr` loads for any `gh pr` create, update, comment, or similar mutating PR action, including forks. Oracle and imaging stay available from their skill and agent descriptions. The driving agent performs implementation, tests, fixes, and repository operations directly.
...
- `/skill:dev-plan <task>` remains available for planning-only work. Keep the plan local unless the user explicitly asks to register it in Doct.
...
- Use normal OMP mode for planning and same-session implementation. The driving OMP session uses `devin/swe-2:high` by default for implementation, scoped review, and PM outcome review; use `openai-codex/gpt-5.6-terra:high` when correctness depends materially on technical judgment. Unresolved consequential choices escalate to Oracle rather than routing implementation through Sol. Do not enable native plan mode, launch Pi, require a Pi implementation profile, or use Pi slash commands.
