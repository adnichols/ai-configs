...
- Required code reviews run through the active harness's configured `reviewer` subagent. In a Herdr delivery run, additionally require the visible labeled-tab Pi completeness reviewer on `xai/grok-4.6:high` to return `COMPLETE` against the plan before local merge readiness; it is a read-only plan-completeness loop, not a substitute for the code-review gate. Do not make Codex or Claude Code a required review transport.
...
- **Remote configuration installs:** Treat Git as the only transport for repository-managed configuration. Commit and push the change, install locally, then on each remote host run `git pull --ff-only` in its ai-configs checkout before installing from that checkout. Do not use tar, rsync, scp, or ad hoc copied source trees for remote installs.
...
- Use the configured OMP `@planner` and `@reviewer` contracts for readiness and autoreview, and `@completeness` on `xai/grok-4.6:high` for completeness. OMP completeness is accepted only through the request-bound envelope from `delivery completion-review --prepare`; no Pi/Grok witness tab is involved.
