...
- `/run-plan <plan>` / `/skill:run-plan <plan>` is the full implementation-through-ready-PR workflow for an existing execution-ready reviewed plan: implementation, implementation-stage PM review, the active-harness reviewer-subagent pre-PR review, and—when delivery runs in Herdr—a visible labeled-tab Pi/Grok 4.5 completeness-review loop to `COMPLETE`, followed by base freshness, PR creation, current PR feedback snapshot, and local merge-readiness consensus.
