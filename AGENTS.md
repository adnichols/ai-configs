...
- **Task Lists & Plans**
  ...
  - For BDD/TDD execution plans:
    ...
    - Use one implementation review plus one targeted rereview after fixes by default. Allow a third round only for a new concrete blocker introduced or exposed by the fix, then stop and report the specific finding that stayed unresolved and why. Count one budget across pre-PR and post-PR review; PR existence never resets or changes the limit.
