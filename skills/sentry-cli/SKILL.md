...
name: sentry-cli
version: 0.45.0
description: Investigate Sentry issues, events, traces, logs, and releases with the `sentry` CLI (skill name sentry-cli). Use when the user wants to inspect Sentry orgs/projects, production incidents, unresolved errors, recent events/tags/releases, Session Replay, user feedback, or to resolve/unresolve/archive issues after confirmation. Also use for `sentry docs`, `sentry schema`, and authenticated Sentry API calls.
requires:
  bins: ["sentry"]
  auth: true
---
...
Help users interact with Sentry from the command line using the `sentry` binary (not the legacy `sentry-cli` command). The skill name stays `sentry-cli`.
...
- **Binary is `sentry`** — commands are `sentry <noun> <verb>`. Do not call `sentry-cli`.
...
- Always confirm with the user before running destructive commands: `project delete`, `trial start`, `release delete`, `alert issues delete`, `alert metrics delete`
- Investigate first. Do not change issue state unless the user explicitly asks.
- Before `sentry issue resolve`, `unresolve`/`reopen`, `archive`, or `merge`: show the exact command, identify org/project/issue, ask for confirmation, and only then run it.
- Do not run bulk issue mutations unless the user clearly asked for a bulk action.
- Do not assume an environment filter unless the user asked for one. If they say "production", add `environment:production` to `--query`.
- For mutations, verify the org/project context looks correct in the command output before proceeding with further changes
- Never store or log authentication tokens — the CLI manages credentials automatically. Do not print `sentry auth token` output.
