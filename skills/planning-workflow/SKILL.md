...
- Load `html-plan-reviewer` before writing, registering, linking, updating, or monitoring any `thoughts/plans/*.html` artifact. Use its Doct-backed `doct-agent plans` instructions for reviewer-facing HTML plans, with `https://doct.nodaste.com` as the default registration endpoint. After registration, follow the returned `listenerInstructions` and start the durable listener before browser-review handoff.
...
- if repo-local guidance requires a plan review registration, prefer the Doct-backed `html-plan-reviewer` flow and verify the returned listener is running before browser-review handoff; use a checked-in/local plan server only when the repo or user explicitly requires that legacy surface,
