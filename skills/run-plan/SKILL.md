...
In Pi, proactively invoke the repository-owned read-only `oracle` subagent when implementation exposes one consequential decision that the reviewed plan and targeted repository evidence do not settle: competing ownership or architecture choices, hard-to-reverse exact-contract/schema/migration decisions, conflict with locked plan decisions, or a newly exposed tradeoff that could require a plan pivot. Give Oracle the bounded decision, plan constraints and inherited decisions, current evidence and files, credible options, the driving agent's recommendation, uncertainty, and one narrow question. Verify its claims and record the accepted or rejected disposition in the coverage ledger and plan decisions/deviations log when it changes the execution path.

Oracle cannot authorize scope expansion or make a product choice. If it identifies a user/product/scope decision, stop for the operator. It does not replace the `planner` readiness evidence, PM outcome review, required `reviewer`, completeness review, tests, or any delivery stage. Do not use it for routine implementation questions or repeatedly consult it without materially new evidence.
...
`delivery completion-review` creates a labeled sibling tab in the same Herdr workspace without moving operator focus, starts a new interactive Pi on that tab's root pane in the same live worktree with `--model xai/grok-4.5:high`, and sends its full visible review prompt. The reviewer may read the plan, current diff, status, and validation artifacts but must not edit files or execute verification.

The driving agent must read the labeled completeness tab's verdict. For every `FINDINGS_TO_RESOLVE` item that is `IN_PLAN`, `PLAN_PREREQUISITE`, or `REGRESSION_FROM_THIS_DIFF`, make the smallest in-scope correction, run invalidated verification, and request the same named reviewer's next round:
