...
When the request transfers live work to another agent, load `skill://herdr-agent-handoff` before creating or opening any destination session, worktree, workspace, tab, or pane. The handoff skill owns the default current-session-plus-new-worktree topology, same-host and remote routing, runtime and model fidelity, context transfer, callback availability, and ownership change. This skill remains the CLI and resource-control authority.
