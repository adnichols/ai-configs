...
5. Load relevant skills:
   - `html-plan-reviewer` before writing, registering, linking, updating, or monitoring any `thoughts/plans/*.html` artifact; use its Doct-backed `doct-agent plans` workflow against `https://doct.nodaste.com` for reviewer-facing HTML plans, including the returned listener startup instructions after registration
