---
name: dev-plan
description: Materialize or update a single-file execution plan after discovery. Keep the plan local unless the user explicitly asks to register it in Doct. Planning-only work - synthesizes validated research into an actual execution plan without modifying product code.
---
...
5. Load relevant skills:
   - `doct-document-ops` only when the user explicitly asked to register, publish, update, or monitor a plan in Doct; use its `doct-agent plans` workflow against `https://doct.nodaste.com`, including the returned listener startup instructions after registration. Doct plans are HTML-only.
...
- Follow the canonical `planning-workflow` integration-integrity planning contract. When triggered, load `integration-integrity` and include its Contract and distributed-integration inventory. Do not add an empty inventory when no trigger applies.
