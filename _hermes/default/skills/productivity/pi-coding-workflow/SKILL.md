...
- Start pi deterministically. Do **not** rely on pi's remembered last-used interactive model/provider. Constrain default launches to the `openai-codex` family and default to the model/provider in `~/.pi/agent/settings.json` when available (currently `openai-codex-4` + `gpt-5.5` on Aaron's machine), resolving a concrete provider that actually offers that model and passing it explicitly at launch.
