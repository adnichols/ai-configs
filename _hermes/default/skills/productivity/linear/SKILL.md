...
## Repo-local ltui configuration

When working inside Aaron's code repos, check repo-local Linear configuration such as `.ltui.json`. The local `ltui` profile owns authentication independently of the Hermes process environment.
...
- If `ltui issues create` errors after submission with a transient Linear/network/project-fetch failure (for example HTTP 503), do **not** immediately retry creation. First search/list by the exact title in the target team/project to see whether the issue was created despite the error. Only retry if verification shows no issue. If a retry already created a duplicate, cancel or duplicate-link the extra issue, remove automation labels like `autobuild` from the extra, and comment pointing to the canonical issue.
