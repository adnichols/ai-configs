...
- Final-output plugin hooks like `transform_llm_output` run after the model turn completes. They can rewrite final text but cannot guarantee that progressive streaming/draft frames did not already expose disallowed text. For hard outbound guarantees, either buffer streaming until vetted or guard both the final-send path and the streaming/draft path.

## Reference patterns
- See `references/outbound-response-guard.md` for a session-derived pattern for deterministic response guards, including final-send filtering, streaming prefix holdback, and regression tests.
