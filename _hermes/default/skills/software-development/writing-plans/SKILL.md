---
name: writing-plans
description: Use when creating or updating executable software plans. Keep plans as local files unless the user explicitly asks to register them in Doct.
version: 2.0.0
author: Hermes Agent
license: MIT
metadata:
...
## Default: plans stay local

When the user asks to create, write, or update a plan, write a local plan file. Do not register it in Doct.

Register in Doct only when they explicitly ask to send, publish, register, or review the plan in Doct, or invoke `reviewed-html-plan` / `send-plan-to-doct`. Then write HTML, register through `doct-document-ops`, start the comment listener, and share the canonical Doct URL.

Resolve the source path from repo-local guidance or an existing plan path supplied by the user. If local guidance does not define the active artifact and the user did not supply an existing plan path, ask one targeted question and stop. Do not assume HTML or Markdown.
...
- Load `doct-document-ops` only when the user explicitly asked to register, publish, update, or monitor a plan in Doct. Use `https://doct.nodaste.com` as the registration endpoint.
