...
- Load `doct-document-ops` before writing, registering, linking, updating, or monitoring any reviewer-facing `thoughts/plans/*.html` artifact. All reviewer-facing Doct plans are **HTML**; do not produce or register Markdoc plans. Use its Doct-backed `doct-agent plans` instructions for reviewer-facing plans, with `https://doct.nodaste.com` as the default registration endpoint and `--source-format html`. After registration, follow the returned `listenerInstructions` and start the durable listener before browser-review handoff.
