...
description: Shared planning doctrine for creating or updating executable software plans, including the default preference to register HTML/Markdoc plans in Doct through `doct-agent plans` on `https://doct.nodaste.com`. Use when moving from read-only research into writing a real plan, structuring a resumable TDD/BDD implementation plan, or when a command like `dev:plan` needs canonical planning workflow and skill-routing guidance.
---
...
- Load `html-plan-reviewer` before writing, registering, linking, updating, or monitoring any `thoughts/plans/*.html` or `thoughts/plans/*.markdoc` artifact. Use its Doct-backed `doct-agent plans` instructions for reviewer-facing HTML/Markdoc plans, with `https://doct.nodaste.com` as the default registration endpoint.
