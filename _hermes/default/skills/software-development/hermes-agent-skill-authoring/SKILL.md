...
## Bulk Runtime Skill Migrations

When updating several installed skills in `~/.hermes/skills` or migrating a workflow across skills, use the checklist in `references/bulk-skill-migration-checklist.md`. The key discipline is: back up touched runtime skill directories first if the tree is not under git, patch active instructions rather than only descriptions, validate every touched `SKILL.md` frontmatter, verify command examples against CLI help, and search both for old default-path commands and for the new canonical commands before reporting completion.

Treat remaining legacy terms as acceptable only when the text explicitly labels them as historical, deprecated, or fallback-only. If an old command still reads like the default workflow, patch it.
