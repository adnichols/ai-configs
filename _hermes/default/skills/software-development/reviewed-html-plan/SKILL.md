...
Use this skill when the user wants the planning and review process completed before implementation starts. The output is a single reviewed HTML/Markdoc-sourced plan that is registered in Doct through `doct-agent plans` on `https://doct.nodaste.com` and either marked execution-ready or explicitly blocked on a product/scope decision.
...
- a plan slug,
- an existing `thoughts/plans/<slug>.html` or `thoughts/plans/<slug>.markdoc` path,
...
Resolve to one canonical plan source path. Prefer `thoughts/plans/<slug>.markdoc` when repo guidance defines Markdoc as the editable source; otherwise use `thoughts/plans/<slug>.html` for handcrafted/legacy HTML.

```text
thoughts/plans/<slug>.<markdoc|html>
```

Use lowercase, digits, and hyphens for the slug. If the user gave an existing Markdown plan, read it as source input but write the reviewed artifact as Markdoc when repo guidance supports it, otherwise as HTML. Do not preserve Markdown-only planning unless explicitly requested.
...
### 2. Create or refresh the HTML/Markdoc plan

Write or update the resolved source path. For `.markdoc`, use the repo/Doct Markdoc plan format; for `.html`, use semantic HTML, not Markdown renamed as HTML.
...
Plan: thoughts/plans/<slug>.<markdoc|html>
