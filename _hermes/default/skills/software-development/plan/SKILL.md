...
metadata:
  hermes:
    ...
    related_skills: [writing-plans, planning-workflow, html-plan-reviewer, reviewed-html-plan]
---
...
5. Load relevant skills:
   - `html-plan-reviewer` before writing, registering, linking, updating, or monitoring any `thoughts/plans/*.html` or `thoughts/plans/*.markdoc` artifact; use its Doct-backed `doct-agent plans` workflow against `https://doct.nodaste.com` for reviewer-facing HTML/Markdoc plans
