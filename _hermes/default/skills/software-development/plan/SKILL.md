---
name: plan
description: "Plan mode for Hermes: inspect context and write/update the active plan artifact without executing. Keep the plan local unless the user explicitly asks to register it in Doct."
version: 2.0.0
author: Hermes Agent
license: MIT
metadata:
...
5. Load relevant skills:
   - `doct-document-ops` only when the user explicitly asked to register, publish, update, or monitor a plan in Doct; use its `doct-agent plans` workflow against `https://doct.nodaste.com` for HTML plans
...
- If the user explicitly asked to register, update, or monitor the plan in Doct, use `doct-document-ops` as the sole source for current Doct commands: auth/context checks, `doct-agent plans register --base-url https://doct.nodaste.com --source-format html --title '<Plan Title>'`, canonical Doct URLs, `plans update`, plan queue inspection, agent claims, ack/resolve, and final readiness/status updates.
