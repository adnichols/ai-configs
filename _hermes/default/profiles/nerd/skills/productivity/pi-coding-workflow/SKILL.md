...
- Start pi deterministically. Do **not** rely on pi's remembered last-used interactive model/provider. Constrain launches to the `openai-codex` family and default the model to `gpt-5.4`, resolving a concrete `openai-codex*` provider that actually offers that model and passing it explicitly at launch.
...
- After launching, verify the live pi header in tmux before trusting the worker. The header must show `gpt-5.4` and an active provider in the `openai-codex` family. If it does not, stop immediately and repair the launcher instead of continuing the workflow.
...
- Start pi deterministically with an explicit model selection. Default the launcher to `--model gpt-5.4` instead of inheriting pi's last-used interactive model; allow explicit overrides only through launcher-controlled settings such as `PI_MODEL` / `PI_PROVIDER`.
