...
- If using Aaron's authenticated `ltui` CLI for issue-state changes, verify persistence with a fresh `ltui --no-agent --format json issues view ...`; default agent-mode `ltui issues update ...` can print a successful target state before the viewed issue reflects it. If verification stays stale, rerun the update with `ltui --no-agent issues update ...` and verify again.
