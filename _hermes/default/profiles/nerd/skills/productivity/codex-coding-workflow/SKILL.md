...
description: "Legacy Codex-based workflow for substantive repo work under ~/code/. Aaron's current default for Linear-backed OpenCode work is hermes-opencode-linear-build; use this only when he explicitly asks for Codex or when OpenCode is unavailable/unsuitable."
version: 0.1.1
author: Hermes Agent
license: MIT
metadata:
...
Use this when Aaron explicitly asks for Codex, when OpenCode is unavailable, or when comparing against the previous Codex workflow. The preferred current Linear-backed OpenCode workflow is `hermes-opencode-linear-build`, which delegates orchestration to OpenCode's `/cmd:linear-build-workspace`.

It preserves the proven Pi workflow discipline, but drives implementation through OpenAI Codex CLI instead of Pi.

This is no longer the default for new repo-based implementation if Aaron asks for the OpenCode workflow.
...
- Codex CLI is available as `codex`; current verified CLI supports `codex exec`, `-C/--cd`, `-s/--sandbox`, `-a/--ask-for-approval`, `--output-last-message`, and `codex review --base/--uncommitted`.
...
- Use `codex exec` for non-interactive work. For repo-local autonomous edits prefer `-s workspace-write -a never`; avoid `danger-full-access` or `--dangerously-bypass-approvals-and-sandbox` unless Aaron explicitly accepts the risk.
- Use Codex and Claude Code as fresh, independent read-only plan reviewers. Before implementation, both must explicitly agree the latest plan is execution-ready. Prefer Claude print mode (`claude -p`) with read-only tool permissions for Claude review gates; do not let review passes edit files.
...
```bash
codex exec -C <repo> -s workspace-write -a never '
...
```bash
codex exec -C <repo> -s read-only -a never --output-last-message /tmp/codex-plan-review.md '
...
Use EXECUTION_READY only if the plan is specific, scoped, testable, sequenced correctly, and safe to implement without unresolved product/technical ambiguity.
If NOT_READY, list blockers, gaps, ambiguities, missing validation/evidence, and concrete plan changes required.
' --allowedTools 'Read,Bash(git diff*),Bash(git status*)' --max-turns 20 > /tmp/claude-plan-review.md
```
...
```bash
codex exec -C <repo> -s read-only -a never --output-last-message /tmp/codex-pm-plan.md '
...
```bash
codex exec -C <repo> -s read-only -a never --output-last-message /tmp/codex-pm-implementation.md '
...
```bash
git diff <base-branch>...HEAD | claude -p '
Review this branch diff thoroughly for correctness, security, data-loss risk, concurrency/race issues, missing tests, maintainability, and repo-convention violations.
Return prioritized findings with file/line references where possible. Do not edit files.
' --allowedTools 'Read,Bash(git status*),Bash(git diff*)' --max-turns 8 > /tmp/claude-code-review.md
```
...
```bash
codex exec -C <repo> -s read-only -a never --output-last-message /tmp/codex-code-review.md '
