...
Print mode runs a one-shot task, returns the result, and exits. In generic CLI use it does not require interactive dialogs, but **when invoking Claude Code from Hermes on Aaron's macOS setup, run it through an interactive login shell and allocate a PTY** so shell startup loads the authenticated Claude Max environment.

```
terminal(
  command="HOME=/Users/anichols zsh -lic 'cd /path/to/project && claude -p \"$(</tmp/prompt.txt)\" --allowedTools \"Read\" --max-turns 20'",
  pty=true,
...
For auth checks on that setup, also use the same pattern:

```
terminal(command="HOME=/Users/anichols zsh -lic 'claude auth status --text'", pty=true, timeout=60)
```

Using `zsh -l -c` without `-i`, or running without PTY, can incorrectly report `Not logged in` even when the interactive Claude Code login is valid.
...
13. **`error_max_turns` means the task may be unfinished, not failed validation** — rerun with a higher `--max-turns` or narrow the prompt before trusting the result. For PR reviews on Aaron's machine, `--max-turns 20-25` is a safer default than 8-10 when Claude must inspect diffs and tests.
14. **Long foreground print-mode runs can look stuck** — redirect output to a `/tmp/...md` artifact, set a realistic timeout, and if interrupted, inspect the artifact and live process list before retrying. Do not leave Aaron without a status update during long review/check waits.
