...
### In-Repo vs User-Local Skills
- **User-local:** `~/.hermes/skills/<category>/<name>/SKILL.md` — created via `skill_manage(action='create')`
- **In-repo:** `skills/<category>/<name>/SKILL.md` — committed, shipped with the package. Use `write_file` + `git add`
...
3. Validate locally (YAML frontmatter, size limits)
...
- Current session won't see the new skill until `/reset`
...
### When to Use
- Adding/removing static services in the Hermes Docker image
- Diagnosing why a per-profile gateway isn't starting or surviving restart
- Modifying `cont-init.d` boot scripts or per-profile gateway run-scripts
...
│   └── 02-reconcile-profiles ← container_boot.py
├── s6-rc.d (static services)
│   ├── main-hermes/run      ← exec sleep infinity (no-op slot)
│   └── dashboard/run        ← conditional
└── /run/service (per-profile gateways, tmpfs)
    └── gateway-<name>/
        ├── run  ← s6-setuidgid hermes hermes -p <name> gateway run
        └── log/run
```

### Why Architecture B (CMD as main program)
1. `cont-init.d` scripts receive no CMD args
2. s6 halt always exits 143 regardless of written exitcode

So main hermes runs as the CMD (`/opt/hermes/docker/main-wrapper.sh`), not as a supervised service. Per-profile gateways under `/run/service/` get full supervision.

### Quick Recipes
```bash
docker exec <c> /command/s6-svstat /run/service/gateway-<name>   # Inspect
docker exec <c> /command/s6-svc -u /run/service/gateway-<name>   # Up
docker exec <c> /command/s6-svc -d /run/service/gateway-<name>   # Down
docker exec <c> tail -n 50 /opt/data/logs/container-boot.log     # Reconciler log
```

### Pitfalls
- `docker exec` defaults to root; files under `$HERMES_HOME/profiles/` should be hermes-owned
- Service slot on tmpfs → wiped on container restart; reconciler recreates it
- Gateway starts then exits → profile likely unconfigured; fix config, supervisor will retry
...
2. Ensure `subcommands` tuple matches TUI expectations
3. Add handler in `cli.py::process_command()` for CLI-only
4. Add handler in `gateway/run.py` for gateway-available

### Common Issues
1. **Shows in TUI but not autocomplete** → missing from `COMMAND_REGISTRY`
2. **Shows in autocomplete but doesn't work** → missing frontend or gateway handler
3. **Behavior differs between CLI and TUI** → divergent implementations
4. **Config persists but UI doesn't update** → patch nanostore state immediately (`patchUiState`)

### Debugging Tactics
- Python side hangs → use `python-debugpy` to break inside `_SlashWorker.exec`
- Ink side not reacting → use `node-inspect-debugger` to break in `app.tsx`
- Registry mismatch → compare `COMMAND_REGISTRY` against TUI local command list

### Verification
1. Rebuild TUI: `npm --prefix ui-tui run build`
2. Test autocomplete with `/`
3. Confirm behavior, persisted config, and live UI state
