...
- Outlines: Need JSON schema validation with zero-overhead FSM filtering
- LMQL: Prefer declarative query syntax

---

## Outlines: Zero-Overhead Structured Generation

Outlines (dottxt.ai) is a specialized alternative that uses **Finite State Machines (FSM)** at the token level to guarantee structured outputs with **zero overhead**.

### When to Use Outlines

- **Pydantic-native workflows**: Define output schemas as Pydantic `BaseModel` classes and get validated Python objects directly
- **JSON schema generation**: Automatically convert JSON schemas to CFG → FSM for guaranteed valid JSON
- **Maximum inference speed**: Zero overhead structured generation (1.2–2× faster than post-validation)
- **Local model deployments**: First-class support for Transformers, llama.cpp, and vLLM backends

### Quick Example: Pydantic Structured Output
...
# Generate a valid Python object, guaranteed
prompt = "Extract user: John Doe, 30, john@example.com"
...
| Outlines Feature | How It Works |
|-----------------|--------------|
| **FSM Filtering** | Schema → CFG → FSM; invalid tokens masked at each step |
| **Fast-Forward** | Deterministic paths (e.g., `{`, `"name"`) are skipped instantly |
| **Type-Safe Generators** | `outlines.generate.choice()`, `.json()`, `.regex()`, `.integer()` |
| **Backend Flexibility** | Transformers, llama.cpp, vLLM, OpenAI (limited) |

### Backend Example: vLLM + Outlines
...
# Zero-overhead structured generation at serving time
prompt = "Extract: iPhone 15 Pro, $999, available"
product = outlines.generate.json(model, Product)(prompt)
```

### Guidance vs Outlines Decision Tree

| Requirement | Recommendation |
|-------------|----------------|
| Need complex control flow (loops, conditionals) | **Guidance** |
| Need Pydantic models with automatic validation | **Outlines** |
| Need token healing | **Guidance** |
| Need maximum inference speed (zero overhead) | **Outlines** |
| Need API model support (OpenAI, Anthropic) | **Guidance** |
| Need local model support (llama.cpp, vLLM) | Both work; **Outlines** for speed |
