...
## Unsloth: 2–5× Faster LoRA/QLoRA

Unsloth is an optimized drop-in replacement for standard PEFT/LoRA training that provides **2–5× speedup** and **30–80% VRAM reduction** while maintaining identical accuracy.

### When to Use Unsloth

- **Speed is critical**: Iteration cycles, large datasets, or tight deadlines
- **VRAM-constrained hardware**: Train larger models on the same GPU (e.g., 70B on 24GB)
- **Compatible architectures**: Llama, Mistral, Gemma, Qwen (check [docs](https://docs.unsloth.ai) for latest)

### Installation

```bash
pip install unsloth
# Or with specific PyTorch/CUDA version:
pip install "unsloth[colab-new] @ git+https://github.com/unslothai/unsloth.git"
```

### Quick Start: Unsloth LoRA
...
# Add LoRA adapter (drop-in replacement for PEFT)
...
# Train with standard TRL or HuggingFace Trainer
...
|--------------|--------|-------|
| **Manual backprop** | 2–3× speedup | Custom CUDA kernels for LoRA grads |
| **Flash Attention 2** | 1.5–2× speedup | Enabled automatically if available |
| **Optimized kernels** | 30% less VRAM | Fused ops, reduced intermediate allocations |
| **GRPO support** | 2–3× speedup | `FastLanguageModel` + `GRPOTrainer` |
...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    max_seq_length=1024,
    load_in_4bit=True,
    fast_inference=True,  # Enable for GRPO
)
...
# Identical GRPO workflow, 2–3× faster
trainer = GRPOTrainer(
    model=model,
    reward_funcs=[format_reward, correctness_reward],
    args=GRPOConfig(output_dir="./grpo-unsloth", num_generations=8),
    train_dataset=dataset,
)
trainer.train()
```
...
- **Architecture coverage**: Only Llama, Mistral, Gemma, Qwen, and a few others (not universal)
- **Newer method**: Less battle-tested than vanilla PEFT in some edge cases
- **Installation**: May require specific CUDA/PyTorch versions

### Resources

- **Docs**: https://docs.unsloth.ai
- **GitHub**: https://github.com/unslothai/unsloth
- **Colab**: https://colab.research.google.com/drive/1Dyauq4kTZoLewQ1cApceUQVNcnnNTzg_
