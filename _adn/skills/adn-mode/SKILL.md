...
Remaining triggers:
- OMP `eval` tool: scratch analysis only (counts, parses, probes). Never create or patch tracked files through it (`Path.write_text`, prelude `write()`, `Bun.write` to a repo path). The **Eval** playbook below is a blind skill A/B test, not that tool. `edit` for surgery, `write` for new files, `bash` for a committed lever.
