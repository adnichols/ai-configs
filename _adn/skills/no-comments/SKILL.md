...
3. The driving agent applies accepted removals and the smallest authorized root-cause fixes. If a constraint can be encoded in types, validation, or a test within scope, do so and remove the redundant comment. Keep an accurate comment when encoding is deferred. Do not widen the task into an architectural rewrite. Never bolt on symptom guards.
