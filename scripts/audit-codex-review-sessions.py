...
# `PASS` is the only green verdict emitted going forward. Legacy green and
# scope-worded tokens stay accepted so historical artifacts keep validating:
# `CLEAN_FOR_PR`/`PASS_SCOPED`/`PASS_WITH_DOCUMENTED_OUT_OF_SCOPE_FOLLOW_UPS`
# read as green, `FIX_IN_SCOPE_FINDINGS` as `FINDINGS_TO_RESOLVE`, and
# `BLOCKED_BY_SCOPE_QUESTION` as `BLOCKED_BY_QUESTION`. `PLAN_EXECUTION_READY`
# is unchanged.
PROFILE_TOKENS = {
