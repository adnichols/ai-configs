...
type CommandMap = Record<
	string,
	{ description: string; handler: (args: string, ctx: any) => any }
>;
...
beforeAll(async () => {
	...
	logDir = await mkdtemp(join(tmpdir(), "percentage-compaction-log-test-"));
...
describe("percentage-compaction extension", () => {
	test("does not auto-compact at 60%; sends a model-visible soft nudge", async () => {
		...
		expect(sentMessages[0].message.content).toContain(
			"not an instruction to compact immediately",
		);
		...
		expect(
			notifications.some((entry) => entry.message.includes("soft nudge")),
		).toBe(true);
	});
	...
	test("does not auto-compact at 75%; sends one rate-limited strong nudge", async () => {
		...
		expect(sentMessages[0].message.content).toContain(
			`${HARD_AUTO_COMPACTION_PERCENT}% is the hard automatic backstop`,
		);
	});
	...
	test("compact_context queues marker plus JSON intent for the next safe post-tool boundary", async () => {
		...
		expect(compactCalls[0].customInstructions).toContain(
			PI_VCC_MANUAL_BYPASS_MARKER,
		);
		expect(compactCalls[0].customInstructions).toContain(
	...
	test("compact_context defers after fresh failure output until an assistant interprets it", async () => {
		...
		await handlers.turn_end?.(
			assistantToolUse(1, [
				toolResult("tc_fail", "bash", "Image not found", true),
			]),
			ctx,
		);
		await tools.compact_context.execute("tc_1", {
	...
	test("failed model-visible nudge delivery is retried instead of marked delivered", async () => {
		const { handlers, sentMessages, notifications, ctx } = setup(60.4, true, {
		...
		expect(
			notifications.filter((entry) =>
				entry.message.includes("nudge delivery failed"),
			),
		).toHaveLength(2);
	});
		...
		await handlers.turn_end?.(
			{
				message: toolResult(
					"tc_2",
					"read",
					"sibling output that must be interpreted",
				),
				toolResults: [],
			},
			ctx,
	...
	test("stale error callbacks cannot clear a newer compaction attempt", async () => {
		...
		percent = 97.0;
		await handlers.turn_end?.(
			assistantToolUse(1, [toolResult("tc_2", "read", "source")]),
			ctx,
		);
		await delay(0);
		expect(compactCalls).toHaveLength(2);
	...
	test("scheduled pi-vcc compaction cancels core overflow and willRetry attempts", async () => {
		...
		await overflow.handlers.turn_end?.(
			assistantToolUse(1, [toolResult("tc_1", "read", "source")]),
			overflow.ctx,
		);
		...
		await willRetry.handlers.turn_end?.(
			assistantToolUse(1, [toolResult("tc_1", "read", "source")]),
			willRetry.ctx,
		);
	...
	test("manual and compact_context no-cut do not auto-resume", async () => {
		...
		await model.tools.compact_context.execute("tc_1", {
			reason: "boundary",
			boundary: "after_test_loop",
		});
		await model.handlers.turn_end?.(
			assistantToolUse(1, [toolResult("tc_1")]),
			model.ctx,
		);
		model.compactCalls[0].onError(
			new Error("Nothing to compact (session too small)"),
		);
		await delay();
	...
	test("core auto-compaction is canceled above hard threshold unless extension-managed", async () => {
		...
		expect(
			notifications.some((entry) =>
				entry.message.includes("repo-managed hard backstop handles 96%"),
			),
		).toBe(true);
	});
	...
	test("core auto-compaction honors same floored no-cut suppression", async () => {
		...
		percent = 96.9;
		const suppressed = await handlers.session_before_compact?.(
			{ customInstructions: undefined },
			ctx,
		);
		...
		expect(
			notifications.some((entry) =>
				entry.message.includes("no safe cut was available at 96%"),
			),
		).toBe(true);
		...
		percent = 96.2;
		await handlers.turn_end?.(assistantStop, ctx);
		const extensionManaged = await handlers.session_before_compact?.(
			{ customInstructions: undefined },
			ctx,
		);
		...
		percent = 97.0;
		const rearmedByUsage = await handlers.session_before_compact?.(
			{ customInstructions: undefined },
			ctx,
		);
	...
	test("core auto-compaction waits for a post-compaction assistant response", async () => {
		...
		expect(
			notifications.some((entry) =>
				entry.message.includes("waiting for the next assistant response"),
			),
		).toBe(true);
	});
	...
	test("core auto-compaction is blocked for the same post-compaction usage value", async () => {
		...
		expect(
			notifications.some((entry) =>
				entry.message.includes("usage is unchanged at 80.123456%"),
			),
		).toBe(true);
	});
	...
	test("fresh failure output suppresses nudges but not hard backstop", async () => {
		...
		await handlers.turn_end?.(
			assistantToolUse(1, [
				toolResult("tc_fail", "bash", "tests failed with error", true),
			]),
			ctx,
		);
		expect(sentMessages).toHaveLength(0);
	...
	test("core overflow retries bypass the extension hard threshold gate only when pi-vcc is loaded", async () => {
		...
		expect(
			missing.notifications.filter((entry) =>
				entry.message.includes("Pi-vcc is not loaded"),
			),
		).toHaveLength(2);
	});
