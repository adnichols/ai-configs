...
usage() {
  ...
  echo "  default: strict installed verification; every installed requirement must exist and match source"
  echo "  --source-only: pre-install source presence/syntax checks without requiring Pi or installed runtime"
}
...
if [[ $source_only -eq 1 ]]; then
  ...
  echo "install state: source-only verification passed"
  exit 0
fi
...
[[ "$registration" == "registered" ]] || { echo "FAIL: package registration missing" >&2; failures=1; }
[[ "$enabled_path" == "enabled" ]] || { echo "FAIL: enabled extension path missing" >&2; failures=1; }
...
echo "install state: source and installed hashes match; registration and extension path verified"
