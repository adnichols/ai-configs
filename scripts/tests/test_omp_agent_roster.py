...
class OmpAgentRosterTest(unittest.TestCase):
    ...
    def test_omp_delivery_model_routing_is_pinned(self):
        ...
        self.assertIn("smol: deepinfra/deepseek-ai/DeepSeek-V4-Flash-0731:high", config)
        self.assertIn("advisor: synthetic/hf:zai-org/GLM-5.3-Flash:high", config)
        self.assertIn("Oracle: openai-codex/gpt-6-astra:medium", config)
        self.assertIn("slow: openai-codex/gpt-5.6-sol:high", config)
        self.assertIn("vision: synthetic/hf:zai-org/GLM-5.3-Flash:auto", config)
        self.assertIn("task: xai-oauth/grok-4.6:medium", config)
        self.assertIn("plan: synthetic/hf:moonshotai/Kimi-K3:high", config)
