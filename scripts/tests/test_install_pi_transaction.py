...
class InstallTransactionTest(unittest.TestCase):
    ...
    def test_bounded_install_and_scoped_check_only(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(foreign.read_text(),'keep');self.assertEqual(foreign_script.read_text(),'keep sibling\n');self.assert_exact_installed_agents(home);installed=json.loads(models.read_text());self.assertEqual(installed['providers']['caller-owned']['apiKey'],'secret');self.assertIn('openai-codex',installed['providers']);self.assertTrue((home/'.agents/skills/autoreview/SKILL.md').is_file());self.assertFalse((home/'.agents/skills/claude-code-review').exists());self.assertFalse((home/'.agents/skills/claude-review-partner').exists());self.assertFalse((home/'.pi/agent/prompts/review:change-claude-code.md').exists());self.assertFalse((home/'.agents/skills/herdr-reviewers').exists());self.assertFalse((home/'.pi/agent/extensions/claude-review').exists());self.assertFalse((home/'.pi/agent/extensions/codex-review').exists());runtime=home/'.agents/scripts/review_orchestration.py';self.assertEqual(runtime.read_bytes(),(ROOT/'scripts/review_orchestration.py').read_bytes());self.assertEqual(stat.S_IMODE(runtime.stat().st_mode),0o755);scripts=home/'.agents/skills/codex-review-partner/scripts';self.assertEqual(stat.S_IMODE((scripts/'process_identity.py').stat().st_mode)&0o500,0o500);self.assertEqual(stat.S_IMODE((scripts/'review_supervisor.py').stat().st_mode)&0o500,0o500);self.assertFalse((home/'.pi/agent/APPEND_SYSTEM.md').exists());before=manifest(home)
            ...
            stale_skill.mkdir(parents=True);(stale_skill/'SKILL.md').write_text('stale\n')
            stale_scoped=subprocess.run(['bash','scripts/verify-pi-install.sh','--scope','pi-review-stack','--check-only'],cwd=ROOT,env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
            self.assertNotEqual(stale_scoped.returncode,0);self.assertIn('removed surface remains',stale_scoped.stderr);self.assertIn('claude-code-review',stale_scoped.stderr)
            import shutil;shutil.rmtree(stale_skill)
    ...
    def extract_install_python(self, interior_marker):
        script = (ROOT / 'install.sh').read_text()
        at = script.index(interior_marker)
