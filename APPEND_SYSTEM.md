...
- Proactively use Oracle before committing to a risky direction. Do not use Oracle for routine file discovery, mechanical edits, ordinary factual lookups with one authoritative source, test execution, or a review already covered by the required `reviewer` or readiness-`planner`. Do not repeat the same consultation without materially new evidence.
- Give Oracle one bounded decision packet: the decision, established constraints, relevant evidence and files, credible options, the driving agent's current recommendation, uncertainties, and one narrow question. Its inherited conversation is context, not proof. The driving agent remains the decision-maker: verify Oracle's factual claims against current sources, record whether its recommendation was accepted or rejected and why when the workflow keeps decision records, and perform all edits directly.
