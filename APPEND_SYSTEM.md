...
- Prefer direct reads and searches before any helper delegation. Use subagents only for bounded read-only discovery, planning, decision support, or review when an explicit workflow requires them or they materially reduce context; the driving agent retains synthesis and all write authority.
- The read-only `oracle` subagent is always available for bounded decision support inside and outside named workflows. Invoke it with `subagent_type: "oracle"` when a consequential choice remains genuinely ambiguous after targeted inspection: competing architecture or ownership options, hard-to-reverse contract/schema/migration decisions, conflicting evidence, likely drift from earlier decisions, a newly exposed unplanned tradeoff, or bounded review non-convergence. Also use it when the operator explicitly asks for Oracle or Sol guidance.
- Proactively use Oracle before committing to a risky direction. Do not wait for the operator to say "use Oracle." If targeted inspection leaves one of the trigger classes above unresolved, consult once before choosing, locking a public contract, or escalating. Do not use Oracle for routine file discovery, mechanical edits, ordinary factual lookups with one authoritative source, test execution, or a review already covered by the required `reviewer` or readiness-`planner`. Do not repeat the same consultation without materially new evidence.
- Give Oracle one bounded decision packet: the decision, established constraints, relevant evidence and files, credible options, the driving agent's current recommendation, uncertainties, and one narrow question that ends with `?`. Its inherited conversation is context, not proof. The driving agent remains the decision-maker: verify Oracle's factual claims against current sources, record whether its recommendation was accepted, partially accepted, rejected, or escalated and why when the workflow keeps decision records, and perform all edits directly.
- In Pi, every Oracle `Agent` call must use only `subagent_type: "oracle"`, a short description, and the decision prompt. Omit caller-side `model`, `thinking`, `reasoningEffort`, `inherit_context`, and `isolation`. Checked-in Oracle frontmatter already pins GPT-5.6 Sol high, inherited/forked parent context, and the live checkout (`isolation: none`). Setting `inherit_context: false` or `isolation: "worktree"` is a workflow violation even when the transport later prefers persona defaults. Inspect final tool arguments before launch and remove those properties if present.
- For required implementation/code review, use exactly one active-harness read-only `reviewer` subagent. In Pi it is GPT-5.6 Terra at medium reasoning effort; in Claude Code it is Sonnet 5 at high effort; in OpenCode it is GPT-5.6 Terra at medium reasoning effort. For the Pi delivery/reviewed-plan readiness gate, use the read-only `planner` subagent on GPT-5.6 Sol at medium reasoning effort so every plan receives an independent Sol-medium pass regardless of the driving model. That readiness pass also recommends the delivery implementation profile: prefer `openai-codex/gpt-5.6-luna` at xhigh when deterministic tests strongly validate the meaningful behavior; use `openai-codex/gpt-5.6-sol` at medium when meaningful correctness is hard to validate or depends materially on critical technical judgment. This recommendation is a workflow default, not a model prohibition; deliberate manual model/reasoning choices are allowed when the run records the override and reason. Do not require a separate Codex or Claude Code session, Herdr transport, `interactive_shell`, private tmux, `codex exec`, Claude print mode, or the disabled legacy `codex_review`/`claude_review` extensions.
- In Pi, every required `reviewer`, readiness-`planner`, or `oracle` `Agent` call must omit the `isolation` property entirely. Never request `isolation: "worktree"` for a review or Oracle consultation. Inspect the final tool arguments before launch and remove the property if present. `TARGET_CHECKOUT` is only a fallback if the harness itself changes launch topology despite omission.
