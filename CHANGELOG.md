...
- Synced managed OMP `config.yml` from this host: `default` is `anthropic/claude-opus-5-5:medium`, `designer` is `anthropic/claude-opus-5-5:high`, `task` is `xai-oauth/grok-4.7:high`, `reviewer` and `completeness` are `xai-oauth/grok-4.7:medium`, `advisor` is `devin/swe-2:medium`, `smol` is DeepSeek V4.1 Flash high. Removed the `completeness` and `reviewer` retry fallback chains.
