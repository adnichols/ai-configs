...
- ADN and OMP now reference model roles, not models. `setup-adn` no longer writes `architect-grok`, `architect-kimi`, or `reviewer-kimi` into `modelRoles` and no longer reports role drift; it reads OMP config and fails closed when a role named by an ADN agent's `model: "@<role>"` frontmatter is undefined. The ADN manifest drops its `roles` block. The OMP `completeness` agent runs `@completeness` instead of the unavailable `xai/grok-4.5:high`, and OMP delivery doctrine (README, run-plan, delivery-run, completeness, planner agent) names the `default` and `completeness` roles. The ADN OMP runtime contract maps leaf-skill model lines to agents and picks panels by distinct configured family.
- Synced managed OMP `config.yml` from this host: `default` is `anthropic/claude-opus-5-5:medium`, `designer` is `anthropic/claude-opus-5-5:high`, `task` is `xai-oauth/grok-4.7:high`, `reviewer` is `devin/swe-2:medium`, `completeness` is `xai-oauth/grok-4.7:medium`, `advisor` is `devin/swe-2:medium`, `smol` is DeepSeek V4.1 Flash high. Removed the `completeness` and `reviewer` retry fallback chains.
